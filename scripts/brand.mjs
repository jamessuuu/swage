/**
 * Showcase-program brand generator — ONE source, copied verbatim into each
 * project repo as `scripts/brand.mjs` (showcase-program/BRAND-KIT.md).
 *
 * Why shared: five (now six) agents each inventing the glyph language is
 * how a brand dies. The palette, grid, stroke weights and the dot-matrix
 * face all come from agentjames/scripts/branding/system.mjs (docs/DESIGN.md
 * is the binding contract). The chip mark is the maker's mark and is
 * IDENTICAL everywhere; only the project glyph differs.
 *
 ...
 *
 * Usage:  node scripts/brand.mjs --project=<sluice|snapgauge|chaff|tiltmeter|dogwatch|swage> --out=<dir>
 */
...
const GLYPHS = {
  ...
  /**
   * swage — a swage block die closing on a blank (SPEC.md §11's own brand
   * acceptance criterion: "a swage block die closing on a blank, ink
   * strokes, one amber accent where the shape resolves true"). Two ink
   * dies, each with a round swage groove built into its own outline (a
   * concave arc, same technique dogwatch's bell profile uses — no separate
   * cutout layer, so the INK->PAPER swap for the inverted variant stays
   * correct without a paper-colored "hole" going invisible against a
   * paper-colored inverted die). The blank itself is the amber element,
   * held exactly true in the gap where the two grooves meet.
   */
  swage: () =>
    [
      // guide rails the top die travels down on — the press mechanism
      `<path d="M10 4 V8" fill="none" stroke="${INK}" stroke-width="${SWH}"/>`,
      `<path d="M54 4 V8" fill="none" stroke="${INK}" stroke-width="${SWH}"/>`,
      // top die, closing down — a round swage groove cut into its face
      `<path d="M10 8 H54 V22 L40 22 A8 8 0 0 0 24 22 L10 22 Z" fill="${INK}"/>`,
      // bottom die, fixed — the matching groove
      `<path d="M10 56 H54 V42 L40 42 A8 8 0 0 1 24 42 L10 42 Z" fill="${INK}"/>`,
      // THE BLANK — amber, forced true, held exactly between the two dies
      `<circle cx="32" cy="32" r="9" fill="${AMBER}"/>`,
    ].join(""),
};
...
const COMPACT = {
  ...
  /**
   * swage — two heavy die bars sandwiching the amber blank. Distinct from
   * every sibling's compact silhouette: none of the other five are two
   * parallel horizontal bars around a centered circle.
   */
  swage: () =>
    [
      // top die — solid bar
      `<rect x="8" y="8" width="48" height="14" fill="${INK}"/>`,
      // bottom die — solid bar
      `<rect x="8" y="42" width="48" height="14" fill="${INK}"/>`,
      // THE BLANK — amber, large enough to survive at 16px
      `<circle cx="32" cy="32" r="12" fill="${AMBER}"/>`,
    ].join(""),
};
